...
public:
   ...
   // Unary operators
   matrixT operator + () _NO_THROW { return *this; }	// Woo changed from this...
   matrixT operator - () _NO_THROW;
   //matrixT& operator + (const matrixT& m) _THROW_MATRIX_ERROR { return *this; }	// ...to these
   //matrixT& operator - (const matrixT& m) _THROW_MATRIX_ERROR;
   //matrixT& operator + (const T& c) _NO_THROW { return *this; }
   //matrixT& operator - (const T& c) _NO_THROW;

   //matrixT operator * () _NO_THROW { return *this; }
   //matrixT operator / () _NO_THROW;
		  
   //matrixT& operator * (const matrixT& m, const T& no) _NO_THROW;	// Woo added these 6 lines. there are functions but they weren't declared
   ...
   matrixT& operator -= (const matrixT& m) _THROW_MATRIX_ERROR;
   matrixT& operator *= (const matrixT& m) _THROW_MATRIX_ERROR;
   matrixT& operator += (const T& c) _NO_THROW;	// Woo added this
   matrixT& operator -= (const T& c) _NO_THROW;	// Woo added this	
   matrixT& operator *= (const T& c) _NO_THROW;
   matrixT& operator /= (const T& c) _NO_THROW;
...
private:
	...
	base_mat (size_t row, size_t col, T** v)
	{
		...
		// Woo added this. initialize matrix values to zero if made for the first time
		if (!v)
		{
			for (size_t i=0; i < row; i++)
...
// combined scalar addition and assignment operator ... Woo added this
MAT_TEMPLATE inline matrixT&
matrixT::operator += (const T& c) _NO_THROW
{
    if (_m->Refcnt > 1) clone();
...
// combined scalar subtraction and assignment operator ... Woo added this
MAT_TEMPLATE inline matrixT&
matrixT::operator -= (const T& c) _NO_THROW
{
    if (_m->Refcnt > 1) clone();
...
// binary scalar addition operator ... Woo added this
MAT_TEMPLATE inline matrixT
operator + (const matrixT& m1, const T& no) _NO_THROW
{
   matrixT temp = m1;
   temp += no;
   return temp;
...
// binary scalar subtraction operator ... Woo added this
MAT_TEMPLATE inline matrixT
operator - (const matrixT& m1, const T& no) _NO_THROW
{
   matrixT temp = m1;
